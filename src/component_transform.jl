struct ComponentTransform{T, Ax} <: VectorTransform
    transformations::T
    axes::Ax
    dimension::Int
...
as(ax::Axis, transformations) = ComponentTransform(transformations, ax)
as(ax::Tuple{Axis}, transformations) = ComponentTransform(transformations, ax)

function transform_with(flag::LogJacFlag, tt::ComponentTransform, x::AbstractVector, index)
    data, index2 = _transform_components(flag, tt, x, index)
    out = ComponentVector(data[begin:end-1], tt.axes)
    ℓ = logjac_zero(flag, eltype(out))
    if flag isa LogJac
        ℓ = data[end]
    end
    return out, ℓ, index2
...
function inverse_eltype(t::ComponentTransform, ::ComponentVector{T}) where {T}
    return T
end

function inverse_at!(x, index, t::ComponentTransform{T}, y::ComponentArray) where {N, T<:NamedTuple{N}}
    for n in N
        yn = getproperty(y, n)
...
convert_comp_to_ttype(::TransformTuple{<:Tuple}, x::Array) = Tuple(x)
function convert_comp_to_ttype(::TransformTuple{<:NamedTuple{N}}, x::ComponentArray) where {N}
    NamedTuple(x)
end



function _transform_components(flag::LogJacFlag, tt::ComponentTransform, x, index)
...
@generated function transform_components!(data, axis, flag::LogJacFlag, transformation::NamedTuple{N}, x, index) where {N}
    exprs = []
    ...
    for k in N
        ...
        push!(exprs, :(($(y_sym), $(ℓ_sym), $(index_sym)) = transform_with(flag, $(trf_sym), x, index)))
...
function ChainRulesCore.rrule(::typeof(_transform_components), flag::LogJacFlag, tt::ComponentTransform, x, index)
    data, index2 = _transform_components(flag, tt, x, index)
    px = ProjectTo(x)
